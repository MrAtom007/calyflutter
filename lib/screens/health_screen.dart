        ...
        LayoutBuilder(
          builder: (context, cns) {
            ...
            return Wrap(
              ...
              runSpacing: gap,
              children: [
                for (final tile in tiles) SizedBox(width: w, child: tile),
              ],
            );
          },
        ),
    ...
    for (final m in layout.enabled) {
      ...
      switch (m) {
        case HealthMetric.heartRate:
          ...
          widgets.add(
            _expandable(context, m, health, _HeroHeart(series: s, bpm: bpm)),
          );
          widgets.add(SizedBox(height: gap));
          break;
        case HealthMetric.bloodPressure:
          final bp = MetricSeries(
            metric: s.metric,
            today: s.today,
            daily: _sliced(s.daily),
          );
          if (bp.daily.length > 1) {
            flushSmall();
            widgets.add(
              _expandable(context, m, health, _bloodPressureCard(context, bp)),
            );
            widgets.add(SizedBox(height: gap));
          }
          break;
  ...
  Widget build(BuildContext context) {
    ...
    return SurfaceCard(
      child: Column(
        ...
        children: [
          SectionHeader(
            title: t('goals').toUpperCase(),
            icon: Icons.flag_rounded,
          ),
          _row(
            context,
            HealthMetric.steps.icon,
            t('hm_steps'),
            health.goalSteps.round().toString(),
            HealthMetric.steps.accent,
            () => _edit(context, health, 'steps', health.goalSteps),
          ),
          _row(
            context,
            HealthMetric.calories.icon,
            t('hm_calories'),
            '${health.goalCalories.round()} kcal',
            HealthMetric.calories.accent,
            () => _edit(context, health, 'calories', health.goalCalories),
          ),
          _row(
            context,
            HealthMetric.sleep.icon,
            t('hm_sleep'),
            '${health.goalSleep.toStringAsFixed(1)} h',
            HealthMetric.sleep.accent,
              ...
              children: [
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        health.googleName ?? health.googleEmail ?? '',
                        maxLines: 1,
                        ...
                        TextButton.icon(
                          onPressed: () async {
                            ...
                            messenger.showSnackBar(
                              SnackBar(
                                content: Text(
                                  ok ? t('backup_saved') : t('backup_failed'),
                                ),
                              ),
                            );
                          },
            ...
            SizedBox(
              ...
              child: OutlinedButton.icon(
                onPressed: () async {
                  ...
                  if (!ok) {
                    messenger.showSnackBar(
                      SnackBar(content: Text(t('sign_in_failed'))),
                    );
                  }
                },
                ...
                onPressed: () async {
                  ...
                  if (!ok && context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text(t('health_connect_failed'))),
                    );
                  }
                },
...
class _SleepCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final last = series.daily.isNotEmpty
        ? series.daily.last.value
        : (series.avg ?? 0);
    final h = last.floor();
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 6),
                Text(
                  below ? t('sleep_status_low') : t('sleep_status_good'),
...
class _MetricDetailSheet extends StatelessWidget {
  ...
  Widget _stat(BuildContext context, String label, String value, Color accent) {
    final c = context.watch<ThemeProvider>().colors;
    return Expanded(
      child: Container(
